...
def maximize_energy(
    ...
    min_possible_cost = sum(float(c[0]) for c in costs)
    max_possible_cost = sum(float(c[-1]) for c in costs)

    if total_cost < min_possible_cost:
        raise ValueError(
            f"Infeasible budget: total_cost={total_cost}, "
            f"minimum possible cost={min_possible_cost}"
        )
    ...
    with torch.no_grad():
        ...
        for batch in data:
            ...
            if isinstance(batch, (list, tuple)):
                ...
                raise ValueError(
                    "Data should be a tuple/list (as in an ImageFolder dataset)"
                )
    model.train(state)
    for h in hooks:
        h.remove()
