...
from factorize_sweep_values import get_values_for_model_and_mode
...
class ImageNetCSeveritySubset(Dataset):
    ...
    def _resolve_corruption_dir(root, imagenetc_subset, corruption):
        ...
        if not subset_root.is_dir():
            raise FileNotFoundError(
                f"Could not find corruption folder '{corruption}' directly under {root} "
                f"or ImageNet-C subset directory {subset_root}."
            )
...
def build_clean_calibration_loader(train_ds, calib_size, batch_size, num_workers, seed):
...
def main():
    ...
    ratios_comp = get_values_for_model_and_mode(args.model_name, args.mode)
